...
/* 
  --- Styles ---
*/
const descriptionContainerClass = 'md:my-0 md:pt-2 md:w-[300px] md:z-20 md:relative mb-4 w-full flex-col-start';
...
const technologiesListClass = 'md:w-[260px] p-2 secondary-sub-paragraph-light-class flex-row-end';

export const ProjectDescription = (props: ProjectDescriptionProps) => {
  return (
    <div
      className={`${descriptionContainerClass} ${
        props.odd ? 'md:left-0 md:justify-start md:items-start' : 'md:right-0 md:justify-end md:items-end'
      }`}
    >
      {/* Project description */}
      <div className={`${descriptionTextClass} ${props.odd ? 'md:text-left' : 'md:text-right'}`}>
        {props.description}
      </div>
